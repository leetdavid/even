 ...
 */
export function parseExpenseCategory(title: string, description?: string): string | null {
  ...
  const categoryMappings = {
    "Food & Dining": [
      "subway sandwich", "burger king", "taco bell", "whole foods", "trader joe",
      "restaurant", "starbucks", "mcdonald", "chipotle", "domino",
      "grocery", "supermarket", "safeway", "cafe", "coffee", "dunkin", "kfc",
      "pizza", "lunch", "dinner", "breakfast", "meal", "food", "snack"
    ],
    "Transportation": [
      "gas station", "rental car", "car rental", "rideshare", "uber", "lyft", "taxi",
      "fuel", "parking", "toll", "metro", "subway", "bus", "train",
      "flight", "plane", "airplane", "airline", "airport"
    ],
    "Entertainment": [
      "amusement park", "netflix", "spotify", "disney", "hulu", "movie", "cinema", "theater",
      "concert", "game", "entertainment"
    ],
    "Health & Medical": [
      "urgent care", "doctor", "hospital", "pharmacy", "cvs", "walgreens", "medicine",
      "medical", "health", "dentist", "clinic", "prescription"
    ],
    "Utilities": [
      "gas utility", "electric", "electricity", "power", "water", "internet",
      "wifi", "phone", "cable", "utility", "bill", "comcast", "verizon", "att"
    ],
    "Home & Garden": [
      "home depot", "home improvement", "cleaning supplies", "lowes", "ikea", "furniture",
      "repair", "maintenance", "garden", "lawn"
    ],
    "Travel": [
      "hotel", "airbnb", "expedia", "accommodation", "resort", "motel", 
      "travel", "vacation", "trip"
    ],
    "Education": [
      "textbook", "university", "college", "tuition", "course", "education",
      "school", "class", "training", "certification"
    ],
    "Personal Care": [
      "personal care", "salon", "haircut", "barber", "spa", "cosmetics", "beauty", "makeup",
      "gym", "fitness", "massage"
    ],
    "Shopping": [
      "best buy", "apple store", "amazon", "target", "walmart", "costco",
      "clothing", "clothes", "shoes", "electronics", "shopping",
      "mall", "store"
    ]
  };
  ...
  let bestMatch = { category: null as string | null, keywordLength: 0, priority: 0 };
  ...
  for (const [category, keywords] of Object.entries(categoryMappings)) {
    ...
    for (const keyword of keywords) {
      if (text.includes(keyword)) {
        ...
        const isSameLengthHigherPriority = keyword.length === bestMatch.keywordLength && priority > bestMatch.priority;
        const isCloseButHigherPriority = Math.abs(keyword.length - bestMatch.keywordLength) <= 2 && priority > bestMatch.priority;
        
        if (isLonger || isSameLengthHigherPriority || (bestMatch.keywordLength > 0 && isCloseButHigherPriority)) {
...
export function describeExpenseChange(
  ...
  switch (field) {
    ...
    case "splitMode":
      const beforeSplitMode = before === "equal" ? "equal split" : before === "percentage" ? "percentage split" : "custom amounts";
      const afterSplitMode = after === "equal" ? "equal split" : after === "percentage" ? "percentage split" : "custom amounts";
      return `Split method changed from ${beforeSplitMode} to ${afterSplitMode}`;

    case "paymentMode":
      const beforePayMode = before === "single" ? "single payer" : before === "percentage" ? "percentage payments" : "custom payments";
      const afterPayMode = after === "single" ? "single payer" : after === "percentage" ? "percentage payments" : "custom payments";
    ...
    case "splits":
      ...
      if (beforeSplits.length === afterSplits.length) {
        const isIdentical = beforeSplits.every((beforeSplit: unknown, index) => {
          const afterSplit = afterSplits[index] as unknown;
          if (!afterSplit || typeof beforeSplit !== 'object' || typeof afterSplit !== 'object') return false;
      ...
      const formatSplits = (splits: Array<{userId: string; amount?: string; percentage?: number}>): string => {
    ...
    case "payments":
      ...
      if (beforePayments.length === afterPayments.length) {
        const isIdentical = beforePayments.every((beforePayment: unknown, index) => {
          const afterPayment = afterPayments[index] as unknown;
          if (!afterPayment || typeof beforePayment !== 'object' || typeof afterPayment !== 'object') return false;
      ...
      const formatPayments = (payments: Array<{userId: string; amount?: string; percentage?: number}>): string => {
