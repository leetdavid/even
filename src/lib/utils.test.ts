...
describe("validatePayments", () => {
  it("should validate single payment for full amount", () => {
    const payments: ExpensePayment[] = [{ userId: "user1", amount: "100.00" }];
  ...
  it("should reject single payment for partial amount", () => {
    const payments: ExpensePayment[] = [{ userId: "user1", amount: "80.00" }];
...
describe("calculateDebts", () => {
  it("should calculate simple debt correctly", () => {
    ...
    const payments: ExpensePayment[] = [{ userId: "alice", amount: "100.00" }];
...
describe("describeExpenseChange", () => {
  ...
  it("should describe split mode changes", () => {
    ...
    expect(result).toBe(
      "Split method changed from equal split to percentage split",
    );
  });
  ...
  it("should describe payment mode changes", () => {
    ...
    expect(result).toBe(
      "Payment method changed from single payer to custom payments",
    );
  });
  ...
  it("should describe split changes with user names", () => {
    ...
    const result = describeExpenseChange(
      "splits",
      beforeSplits,
      afterSplits,
      mockGetUserName,
  ...
  it("should describe payment changes with user names", () => {
    ...
    expect(result).toBe(
      "Payments changed from Alice: 100 to Alice: 60, Bob: 40",
    );
  });
  ...
  it("should handle percentage splits", () => {
    ...
    expect(result).toBe(
      "Splits changed from Alice: 100% to Alice: 60%, Bob: 40%",
    );
  });
  ...
  it("should return null for identical splits with different object references", () => {
    ...
    expect(
      describeExpenseChange("splits", beforeSplits, afterSplits),
    ).toBeNull();
  });
  ...
  it("should work without user name function", () => {
    ...
    const afterSplits = [
      { userId: "alice@example.com", amount: "30" },
      { userId: "bob@example.com", amount: "70" },
    ];
...
describe("parseExpenseCategory", () => {
  describe("Transportation", () => {
    it("should categorize ride sharing services", () => {
      expect(parseExpenseCategory("Uber ride to airport")).toBe(
        "Transportation",
      );
    ...
    it("should categorize car-related expenses", () => {
      expect(parseExpenseCategory("Gas station fill up")).toBe(
        "Transportation",
      );
  ...
  describe("Food & Dining", () => {
    ...
    it("should categorize grocery shopping", () => {
      ...
      expect(parseExpenseCategory("Supermarket shopping")).toBe(
        "Food & Dining",
      );
  ...
  describe("Entertainment", () => {
    it("should categorize streaming services", () => {
      expect(parseExpenseCategory("Netflix subscription")).toBe(
        "Entertainment",
      );
  ...
  describe("Health & Medical", () => {
    it("should categorize medical appointments", () => {
      ...
      expect(parseExpenseCategory("Dentist appointment")).toBe(
        "Health & Medical",
      );
    ...
    it("should categorize pharmacy purchases", () => {
      ...
      expect(parseExpenseCategory("Walgreens medicine")).toBe(
        "Health & Medical",
      );
  ...
  describe("Edge cases", () => {
    ...
    it("should prioritize more specific keywords", () => {
      ...
      expect(parseExpenseCategory("Subway sandwich lunch")).toBe(
        "Food & Dining",
      );
    });
    ...
    it("should use description when provided", () => {
      expect(
        parseExpenseCategory("Random title", "Went to Starbucks for coffee"),
      ).toBe("Food & Dining");
      expect(parseExpenseCategory("Expense", "Uber ride to work")).toBe(
        "Transportation",
      );
    });
    ...
    it("should find keywords within longer text", () => {
      expect(
        parseExpenseCategory("Bought some electronics at Best Buy today"),
      ).toBe("Shopping");
      expect(
        parseExpenseCategory("Had a great meal at the restaurant downtown"),
      ).toBe("Food & Dining");
    });
  });
});
